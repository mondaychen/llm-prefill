...
async function runTest(model: string, provider: string, prompt: string, prefix: string): Promise<string> {
  if (provider === 'openai') {
  ...
  for (const testCase of testCases) {
    for (const model of models) {
      for (const prefix of testCase.prefixes) {
        try {
          const response = await runTest(model.name, model.provider, testCase.prompt, prefix);
          results.push({
            model: model.name,
            prefix,
          ...
          console.error(`Error testing ${model.name} with prefix: ${prefix}`, error);
        }
      }
    }
  }
